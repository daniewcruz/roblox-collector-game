...
## D052 — Consumida documentação oficial da Roblox sobre mecânica/jogabilidade/prototipagem/onboarding
- **DECISÃO**: pesquisada e consumida a pasta oficial de game design do Roblox Creator Hub (17 documentos: core-loops, prototyping, onboarding, quest-design, economias, monetização, LiveOps, UI/UX). Resultado registrado em `14-mecanicas-roblox-oficial.md`. Achado principal: a estrutura de core loop já adotada no GDD (explorar → tocar/despertar → progressão) **bate exatamente** com o framework oficial de 3 camadas (interação minuto-a-minuto → ação mais repetida/mecânica definidora → motor de progressão) — não é invenção nossa, é o padrão que a própria Roblox ensina. A abordagem de prototipagem já planejada (testar candidatos de interação isoladamente, seção 4b) também bate com a recomendação oficial ("prototipagem deve ser rápida e estreita, não a feature inteira"). Onboarding ganhou reforços concretos: expor metas de curto/médio/longo prazo já nos primeiros minutos (não só depois de várias descobertas), limiar de progressão inicial baixo. Achado extra: Character Controller Library (CCL), sistema novo de movimento por habilidades modulares, relevante para os candidatos de Core Gameplay que envolvem mira/movimento customizado — registrado como pendência técnica para a Fase 2.
- **MOTIVO**: usuário pediu pesquisa de documentação já existente sobre mecânica/jogabilidade no Roblox, para fundamentar decisões em fontes oficiais em vez de só raciocínio próprio — mesmo espírito de D051 (usar estudos publicados) aplicado agora à fonte mais autoritativa possível (a própria Roblox).
- **ALTERNATIVAS**: nenhuma — pesquisa confirmatória, não substitutiva.
