...
**Plataforma primária**: Mobile-first (80% das sessões Roblox são mobile — D004; confirmado por fonte oficial adicional em D053, `14-mecanicas-roblox-oficial.md`)
**3 pilares de design oficiais da Roblox aplicados (D053)**: (1) Comportamento — social impulsiona retenção, single-player sofre na plataforma, por isso o cartão de perfil visível (2c) é estrutural, não opcional; (2) Público — jovens priorizam exploração/socialização, "esquisitice e criatividade são centrais à cultura Roblox" (valida D040/D042), polimento visual importa menos que diversão (valida D024); (3) Engine — mobile-first, UI baseada em ícone.
...
### Princípio de dosagem tonal (D046, extraído de um backlog maior — ver `13-backlog-ideias-futuras.md`)

Nem tudo precisa ser sério, nem tudo precisa ser piada — o jogo funciona melhor com **contraste**: momentos de exploração/descoberta focados intercalados com interlúdios leves/cômicos, em vez de caos constante (que cansa) ou seriedade constante (que fica monótono). Aplicação prática, barata, sem exigir sistema novo:
- Eventos "estranhos" (2d) acontecem **programados/ocasionais**, não o tempo todo — a raridade do evento é parte do que o torna especial.
- Regiões futuras (pós-MVP) podem alternar tom: uma mais "séria" visualmente, a próxima mais nonsense — coerente com a lógica de mundo eclético já adotada (D042).
- NPCs (1g) podem ter um lado sério e um lado bobo dependendo da situação, em vez de serem só uma piada constante.

### O que fica para Fase 6+ (visão de longo prazo, não MVP)

- Sistema completo de **papéis/classes jogáveis** (o jogador escolhe um papel, ganha árvore de habilidades própria, desafios cooperativos que exigem 2+ papéis diferentes) — ideia forte, mas escopo grande demais para o MVP enxuto (mesma régua de D011/D019/D022/D044). (D045)
- **Arena de PvP leve, opcional, separada do loop principal** (D047): modo à parte de exploração/coleção — jogador entra por vontade própria, regras simples, mantendo o núcleo do jogo cooperativo/sem-pressão (D013/D020) intacto. Exige sistema de dano/vida/hit detection — trabalho técnico real, não cabe no MVP. Recomendação de tom: manter no registro "brinquedo" já estabelecido (contestos de empurrar/derrubar, não combate realista com armas/sangue) para não contradizer a identidade do jogo nem as salvaguardas éticas de público infantil.
  - **Exemplos concretos de mini-jogos para essa arena/eventos** (D048-D049, confirmados pelo usuário): corrida maluca de blocos (percurso com obstáculos cômicos — molas, patos de borracha, blocos que se mexem); batalha de construção (quem monta a estrutura mais alta/criativa em tempo limitado, votação da comunidade); caça ao tesouro competitivo (coletar o máximo de itens em tempo limitado, todos recebem alguma recompensa); mini-jogo de pintura (preencher blocos coloridos mais rápido/criativo); batalha de bolinhas de espuma (acertar adversários com objetos não-violentos, tom de brincadeira); **puzzle cooperativo** (2+ jogadores ativam mecanismos/montam peças juntos — reforça o modelo cooperativo, D016); **mini-jogo de dança/expressão** (personagens fazem movimentos engraçados, competição de ritmo/estilo — reforça o pilar de humor, 1g); **arena de combate leve** (armas bobas e não-realistas — martelo gigante, laser colorido — confirma o tom "brinquedo" já recomendado, nunca combate sério); **corrida de veículos** e **desafio de destruição** (derrubar construções de blocos, não "inimigos") — esses dois exigem um pouco mais de sistema (física de veículo/destruição), mas continuam no mesmo bucket de visão Fase 6+, não precisam de backlog separado; **5 modos de fuga coletiva** (D050): bloco gigante rolando atrás do grupo, fuga caótica cooperativa (só avança se ajudarem uns aos outros), fuga musical (chão some no ritmo), fuga de patos gigantes, fuga invertida (perseguir um NPC que foge — já quase idêntico ao evento "Mimo Lendário avistado" de 2d); **mini-jogo de jardim estilo Grow a Garden** (D062) — pausa criativa/calma entre corridas e fugas: plantar e cuidar de um jardim, competição leve por jardim mais colorido/rápido, "super jardim coletivo" cooperativo, eventos surpresa não-violentos (chuva de blocos que viram flores, patos que atrapalham a plantação) — é o contraponto calmo explícito ao ritmo caótico dos outros mini-jogos, reforçando a dosagem tonal (seção 2c). Todos cooperativo-compatíveis ou de competição leve, sem sistema de dano/vida "sério".

**Regra de tom infantil para toda a visão de arena/mini-jogos (D050)**: obstáculos engraçados (molas, blocos que se mexem, NPCs atrapalhados), recompensa para todos mesmo quem não vence, partidas rápidas (mantém energia alta), humor sempre cartunesco — nunca violento. Vale para qualquer mini-jogo futuro desta lista, não só os mais recentes.

**Template de ritmo para corridas/fugas/PvP (D051, última rodada de visão)**: sem estrutura, esses mini-jogos viram só caos aleatório; com estrutura, viram momentos memoráveis. Fluxo recomendado para qualquer um deles:
1. **Preparação** — jogadores entram, instrução rápida (ensinada por ação, não texto longo, mesmo princípio de 2b).
2. **Início leve** — primeiros obstáculos simples, todos se aquecem, **e a ameaça/obstáculo da fase de clímax já aparece de forma segura aqui** (ver nota de pesquisa abaixo).
3. **Caos crescente** — blocos móveis, perseguições, chão desaparecendo.
4. **Clímax** — momento de maior tensão (patos gigantes, chão prestes a sumir todo).
5. **Final** — quem vence ganha mais, mas todos recebem alguma recompensa (reforça a regra de tom infantil acima).
6. **Interlúdio irreverente** — cena engraçada pós-partida (reforça o pilar de humor, 1g, e o princípio de dosagem tonal já adotado).

Este template é só estrutura — zero custo de implementação agora, orienta o design quando esses mini-jogos forem construídos (Fase 6+).

### Fundamentação em estudos reais de design (pedido do usuário: usar análises já publicadas, não só raciocínio próprio)

- **Crash Bandicoot**: análises de design confirmam que o jogo "ensina uma mecânica nova e garante que o jogador a usou" antes de aumentar a dificuldade — nunca segura a mão do jogador, mas é "difícil, porém justo e alcançável", deixando o jogador aprender com o próprio erro. [Medium — Varun Bajaj](https://medium.com/@varunbajaj/crash-bandicoot-designing-games-to-balance-challenge-and-fun-a-game-design-critique-225af9d0661a) A fase icônica de perseguição ("Boulders", pedra gigante rolando) nasceu como variação do mesmo esquema de câmera do jogo, inspirada em *Indiana Jones*. Isso confirma o mesmo princípio "ensinar jogando" já adotado (2b, World 1-1) — não é exclusivo do Mario, é um padrão recorrente em jogos de sucesso.
- **Jogos LEGO (Traveller's Tales)**: a filosofia declarada do estúdio é fazer jogos **acessíveis, engraçados e cooperativos**, voltados a jogadores jovens/não-hardcore — humor é "traço definidor" mesmo em IPs sérias (ex: Batman vira engraçado quando reconstruído em LEGO). Acessibilidade significa que o jogador não precisa memorizar esquemas de controle complexos. [PCGamesN](https://www.pcgamesn.com/lego-marvel-super-heroes-2/travellers-tales-lego-games), [CBR](https://www.cbr.com/travellers-tales-lego-games-20-years-later/) Isso confirma diretamente a filosofia já adotada em D040 (mundo de brinquedos, acessível, cooperativo) — não é uma interpretação nossa, é a filosofia declarada do próprio estúdio que definiu o gênero.
- **Design de sequências de perseguição/fuga**: pesquisa de design de jogos de horror (mesmo princípio se aplica a perseguições não-assustadoras) confirma que sequências de perseguição funcionam melhor quando **a ameaça já apareceu como obstáculo mais cedo**, para o jogador entender o comportamento dela antes da perseguição de verdade começar — introduzi-la de repente é pior design. [Game Developer](https://www.gamedeveloper.com/design/creating-horror-through-level-design-tension-jump-scares-and-chase-sequences) Isso já foi incorporado ao passo 2 do template acima (o obstáculo do clímax aparece de forma segura no "início leve").

**Esta foi a última rodada de expansão de visão combinada com o usuário (D051)** — a partir daqui, o foco volta para validar o Core Gameplay pequeno (seção 4b) e avançar à Configuração Técnica (Fase 2).
- Ambos ficam registrados como visão de expansão social/multiplayer, a desenhar com mais detalhe quando o Core Gameplay pequeno já estiver validado e o jogo tiver tração real. Mecânicas de **plataforma de precisão estilo Crash Bandicoot** (pulos precisos, perseguições, vidas, escudo temporário) foram avaliadas e ficam só no backlog (`13-backlog-ideias-futuras.md`, D048) — são um gênero diferente do verbo central do jogo (D014), exigindo movimento/câmera/checkpoint muito além do escopo técnico atual. O grosso das demais ideias trazidas (bosses, clímax narrativo, dimensão de gravidade, fusão de personagens, mapas-meme) também fica só no backlog, fora do GDD ativo.
