# Game Design Document (GDD) — Fase 1 (revisado, v12 — CHECKPOINT 1 APROVADO, Core Gameplay + identidade visual em validação)
...
- Ambos ficam registrados como visão de expansão social/multiplayer, a desenhar com mais detalhe quando o Core Gameplay pequeno já estiver validado e o jogo tiver tração real. O grosso das demais ideias trazidas (bosses, clímax narrativo, dimensão de gravidade, fusão de personagens, mapas-meme) fica só no backlog separado (`13-backlog-ideias-futuras.md`), fora do GDD ativo.
